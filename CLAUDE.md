...
社内の営業活動に関するドキュメント管理リポジトリ。主に出光興産向けのIT提案書・見積書と、社内ナレッジベースで構成される。ソフトウェア開発プロジェクトではない。
...
- ファイル名に「出光」→ `01_clients/01_idemitsu/`
- 出光向け案件のファイル（IRIS, SWAN, HUB, 海上入出荷, 工場系, MES, IFC, PIMSDB 等）→ `01_clients/01_idemitsu/`
- 新規顧客 → `01_clients/` に連番でフォルダーを作成（例: `02_<顧客名>/`）
- 顧客名が不明 → ユーザーに確認

#### 判定できない場合

ファイル名だけで分類先が判断できない場合は、ユーザーに確認してから移動する。
...
`02_knowledge/01_proposal_cases/案件カタログ_出光興産.md` に新規ファイルの情報を追記する。
...
### 案件横断サマリー（トップレベル）
- **案件横断サマリー_出光興産.md**: 全案件の横断分析・技術スタック一覧・見積パターン分析・ナレッジ活用ガイド
...
1. **まず `案件横断サマリー_出光興産.md`** を読む → 全体像把握
2. **案件名で検索** → `案件カタログ_出光興産.md` でファイル特定
3. **提案内容を確認** → `提案書ナレッジ_出光興産.md` で該当箇所をGrep
4. **見積詳細を確認** → `見積書ナレッジ_出光興産.md` で工数・金額を確認
5. **管理情報を確認** → `管理資料ナレッジ_出光興産.md` でPJ計画・予算を確認
