...
## ローカル対話での使い方（メインの運用方法）

ユーザーがこのフォルダーで Claude Code を起動して「inbox 分析して」等と指示した場合、以下の手順を自動で実行すること。

### 手順

1. **テキスト抽出**: `00_inbox/` に .pptx/.xlsx/.pdf があれば `python scripts/extract_text.py` を実行してテキストを取得する
2. **仕分けルール**（後述）に従って分類を判定
3. **`git mv`** で正しいフォルダーへ移動
4. **ナレッジベース更新**（案件カタログ・見積パターン集・案件横断サマリー）
5. 結果をユーザーに報告

**重要:**
- .pptx/.xlsx/.pdf はバイナリのため直接読めない。必ず `scripts/extract_text.py` で抽出してから分析すること
- Python のパスは `C:\Users\syouta.kawana\AppData\Local\Programs\Python\Python313\python.exe` を使うこと
- 顧客名が不明な場合はユーザーに確認してから移動する
